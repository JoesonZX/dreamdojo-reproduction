...
class LatentActionModel(nn.Module):
    ...
    def __init__(
        ...
        # set True by the trainer when a loss needs to re-decode with a swapped latent
        self.keep_patches = False
        # decoder-exposure noise scale for the reparam trick (α ablation, set by trainer).
        # α=1.0 = standard reparam; α=0.0 feeds z_mu to the decoder during training.
        # ONLY the decoder-fed latent is scaled; KL / action head / L_zero all read z_mu,
        # so this knob isolates "training-time sampling noise seen by the decoder".
        self.decoder_noise_alpha = 1.0
        if contrastive:
    ...
    def encode(self, videos: Tensor) -> Dict:
        ...
        if not self.training:
            ...
            z_rep = z_mu + self.decoder_noise_alpha * torch.randn_like(z_var) * torch.exp(0.5 * z_var)
    ...
    def decode_with(self, patches: Tensor, z_rep: Tensor, H: int, W: int) -> Tensor:
        """Decode the next frame from cached patches and an ARBITRARY latent.

        Factored out of `forward` so interventions can substitute z (zeroed, shuffled,
        swapped) without re-encoding — used by the latent-usage loss and by
        decoder-side evaluation. `patches` is `encode()["patches"]`, `z_rep` has the
        same shape `encode()` produces: [B, T-1, 1, latent_dim].
        """
        video_action_patches = self.patch_up(patches[:, :-1]) + self.action_up(z_rep)
        return unpatchify(F.sigmoid(self.decoder(video_action_patches)), self.patch_size, H, W)

    def forward(self, batch: Dict) -> Dict:
        H, W = batch["videos"].shape[2:4]
        outputs = self.encode(batch["videos"])
        recon = self.decode_with(outputs["patches"], outputs["z_rep"], H, W)
        outputs.update({"recon": recon, "_hw": (H, W)})
        # `patches` is only needed by losses that re-decode with a substituted latent
        # (L_use). Dropping it otherwise keeps peak memory at the pre-refactor level.
        if not getattr(self, "keep_patches", False):
            del outputs["patches"]
        return outputs
