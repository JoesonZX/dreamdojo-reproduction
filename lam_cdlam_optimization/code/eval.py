...
Usage:
  python lam_cdlam_optimization/code/eval.py \
      --checkpoint <out_dir>/step_0020000 \
      --config lam_cdlam_optimization/code/config/exp3_indep.yaml \
...
def load_model(checkpoint_dir, cfg, device):
    ...
    # Training-side extras (e.g. the pctr SigLIP scalars registered on the model by
    # train.py) are not part of the inference architecture — drop them, but stay
    # strict about MISSING keys: those mean a real architecture mismatch.
    model_keys = set(model.state_dict().keys())
    extra = sorted(set(state_dict) - model_keys)
    if extra:
        print(f"[load_model] dropping {len(extra)} training-side keys: {extra[:6]}"
              + (" ..." if len(extra) > 6 else ""))
        state_dict = {k: v for k, v in state_dict.items() if k in model_keys}
    missing = sorted(model_keys - set(state_dict))
    if missing:
        raise RuntimeError(f"[load_model] checkpoint is missing {len(missing)} model keys "
                           f"(architecture mismatch): {missing[:6]}")
