...
#
#   tmux new-session -d -s lamdis_eval 'bash lam_cdlam_optimization/run_eval_all.sh'
#   tmux attach -t lamdis_eval        # check on it (Ctrl-b d to detach)
#
# Results land in lam_cdlam_optimization/results/.
set -uo pipefail

REPO=/home/xuan/embodied-ai
CODE=$REPO/lam_cdlam_optimization/code
OUT=$REPO/checkpoints/lam-dis
RES=$REPO/lam_cdlam_optimization/results
