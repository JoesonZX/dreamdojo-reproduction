...
Encoder 不直接输出一个固定向量，而是输出一个**概率分布的参数**：
- $\mu$（均值向量，32维）：分布的中心
- $\log\sigma^2$（对数方差，32维）：分布的宽度

这意味着"同一个动作"不对应一个固定的点，而是对应一个椭球形区域。这赋予了模型对动作的不确定性建模能力。

**② 重参数化技巧（Reparameterization Trick）**

从分布中采样：$\hat{a}_t = \mu + \sigma \cdot \epsilon$，其中 $\epsilon \sim \mathcal{N}(0, I)$

直接采样无法反向传播（随机操作不可微），但这个写法把随机性移到了 $\epsilon$（一个固定的噪声），$\mu$ 和 $\sigma$ 可以正常计算梯度。这是让 VAE 可以端到端训练的关键技巧。
...
它惩罚编码器把 $\hat{a}_t$ 的分布偏离标准正态。效果是：
- 强迫所有 latent action 的分布"集中"在原点附近
- 压制编码器把过多信息塞进 latent vector
- β=1e-6 非常小，约束很宽松，主要靠重建损失驱动学习

#### 为什么 Encoder 同时看两帧，但 Decoder 只看一帧？

这是 LAM 设计最巧妙的地方：

- **Encoder 看两帧**：才能感知"发生了什么变化"，提取出动作信息
- **Decoder 只看 $f_t$**：被强迫完全依赖 $\hat{a}_t$ 来生成 $f_{t+1}$

如果 Decoder 也看 $f_{t+1}$，它就可以"作弊"——直接复制下一帧而不需要理解动作。只给 $f_t$，Decoder 就必须用 $\hat{a}_t$ 告诉它"接下来会发生什么"，否则无法重建。

**信息瓶颈**：把复杂的帧变化压缩成 32 维，为了在 32 维内重建出 $f_{t+1}$，模型**被迫只保留最关键的信息**——也就是"做了什么动作"，背景颜色、光照变化等无关信息会被丢弃。
